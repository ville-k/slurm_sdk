...
Improve Slurm SDK docs by adding a **decorator-based ** example + **tutorial** that teaches a realistic orchestration pattern: **parallel Train + Eval**, where training progress survives fairshare/timeouts and **may require multiple Slurm jobs per epoch**, while eval runs in parallel and does not block training.
